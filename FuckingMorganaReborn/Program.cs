...
namespace FuckingMorganaReborn
{
    internal class Program
    {
        ...
        {
            Console.WriteLine("FuckingMorganaReborn injected...");
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            _menu = new Menu("FuckingMorganaReborn", "FuckingMorganaReborn", true);
            ...
            foreach (var ene in ObjectManager.Get<Obj_AI_Hero>().Where(x => x.Team != Me.Team))
            {
                ...
                foreach (var lib in FolxuLib.CCList.Where(x => x.HeroName == ene.ChampionName))
                {
            ...
            {
                Console.WriteLine("Exception thrown FuckingMorganaReborn: (BlackShield: {0})", e);
            }
        }
        ...
        internal static void Obj_AI_Base_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            foreach (var ally in ObjectManager.Get<Obj_AI_Hero>().Where(x => x.IsValidTarget(_e.Range, false)))
            {
                ...
                // no linq cus i say :p
                foreach (var lib in FolxuLib.CCList)
                {
